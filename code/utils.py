...
def make_bar_plot(
    ...
    for i in range(len(name_list)):
        ax.bar(
            places[i],
            df.loc[df["name"].str.contains(name_list[i]), setups].mean(),
            label=training_type[i],
...
def make_box_plot(
    df, name_list, training_type=None, colors=None, y_label="", title="", y_lim=None
):
    ...
    box_plot_data = [
        df.loc[df["name"].str.contains(name), :]["var"] for name in name_list
    ]
