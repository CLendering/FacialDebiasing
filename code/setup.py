...
parser.add_argument("--debias_type", type=str, help="type of debiasing used")
parser.add_argument("--path_to_model", type=str, help="Path to stored model")
parser.add_argument("--num_workers", type=int, help="Path to stored model")
...
parser.add_argument(
    "--eval_dataset", type=str, help="Name of eval dataset [ppb/h5_imagenet/h5]"
)
...
def create_run_folder(folder_name):
    ...
    return create_folder_name(
        str(datetime.datetime.now().strftime("%d_%m_%Y---%H_%M_%S"))
    )
...
class Config:
    ...
    # Images to save
    save_sub_images: bool = (
        False if ARGS.save_sub_images is None else ARGS.save_sub_images
    )
    # Hist size
