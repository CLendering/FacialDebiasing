...
class PPBDataset(GenericImageDataset):
    def __init__(
        ...
        **kwargs,
        ...
        self.store: pd.DataFrame = self._apply_filters_to_metadata(
            pd.read_csv(self.path_to_metadata)
        )
    ...
    def init_store(self, path_to_metadata):
        if not os.path.exists(path_to_metadata):
            logger.error(
                f"Path to metadata (and probably PPB) does not exist at {path_to_metadata}!"
            )
            raise Exception

        try:
            store = self._apply_filters_to_metadata(
                pd.read_csv(path_to_metadata, delim_whitespace=True)
            )
            return store
        except:
            logger.error(
    ...
    def _apply_filters_to_metadata(self, df: pd.DataFrame):
        ...
        if len(self.filter_excl_skin_color):
            try:
                ...
                logger.error(
                    "bi_fitz can't be found in the metadata datadframe",
    ...
    def read_image(self, idx: int):
        return Image.open(
            os.path.join(self.path_to_images, self.store.iloc[idx].filename)
        )
